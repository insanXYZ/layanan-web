...
import { Book, Camera, Pencil } from "lucide-react";
...
import { BookDto, UpdateBookRequest } from "@/dto/book-dto";
...
export default function ButtonEditBook({ book }: { book: BookDto }) {
  const [open, setOpen] = useState<boolean>(false);
  const { data: dataRack } = useQueryTanstack(["getRacks"], "/racks");
  ...
  const form = useForm<z.input<typeof UpdateBookRequest>>({
    defaultValues: {
      image: undefined,
      rack_id: book.rack_id ? book.rack_id.toString() : undefined,
      title: book.title,
      quantity: book.quantity,
    },
    resolver: zodResolver(UpdateBookRequest),
  });

  const onSubmit = (data: z.infer<typeof UpdateBookRequest>) => {
    mutate({
      body: data,
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <DialogTrigger asChild>
            <Pencil width={20} className="text-blue-400" height={20} />
          </DialogTrigger>
        </TooltipTrigger>
        <TooltipContent>
          <p>Edit</p>
